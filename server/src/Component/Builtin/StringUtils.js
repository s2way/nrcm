 ...
 */
StringUtils.prototype.firstLetterUp = function (str) {
    if (str.length > 0) {
 ...
 * @return {string} Returns the formatted string
 */
StringUtils.prototype.lowerCaseUnderscoredToCamelCase = function (lowerCaseUnderscored) {
    ...
    for (i = 0; i < parts.length; i += 1) {
        part = parts[i];
        if (part.length > 0) {
            camelCase += this.firstLetterUp(part);
        }
    }
    return camelCase;
};
 ...
 * @return {string} Returns the formatted string
 */
StringUtils.prototype.camelCaseToLowerCaseUnderscored = function (camelCase) {
