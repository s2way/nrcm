...
exports.onRequest = function onRequest(request, response, controller, acl) {
	...
	var body = {"error" : 0, "payload" : "", "contentType" : headerContentType, "attempt" : attempt, "request" : request, "response" : response};
	...
	if (!attempt["acl"]["isAllowed"]) {
		...
		body["request"].on("data", function(data) {
			body["payload"] += data;
		});
		body["request"].on("end", function() {
			try {
				body["payload"] = querystring.parse(body["payload"]);
