...
var NRCM = (function() {  
  ...
  */
  function setUpSync() {
    ...
    setControllerSync(getFileSync(controllerPath));   
    if (!error) {
  ...
  */
  ...
  /**
  * Turn a list of files into controllers sandbox
  *
  * @method setControllerSync
  * @param {Array} files The file list to load as controller
  */
  function setControllerSync(files) {
    ...
    try {
      files.forEach(function(file) { 
        ...
        exec = "controller." + name + " = require('" + fs.realpathSync(file) + "')";
        eval(exec); // import modules into the sandBox
      });   
    } catch (e) {
      error = true;
    }
  ...
  /**
  * Return the object controller
  *
  *@method getController
  *@returns {Object} Returns the controller 
  */
